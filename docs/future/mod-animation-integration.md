...
每个 adapter 只负责把可选模组状态投影为有限的动画输入，例如 Molang query、controller predicate、标准化动作状态或 render-context hint。Adapter 不得拥有动画状态机、修改网络权威、持有 renderer 资源，也不得让模组类型泄漏到核心 animation / processor 接口。
