...
Native 是由 Java 调用的能力层。`com.elfmcys.ysm.natives` 提供 Java 包装，`ysm::lib` 适配 JNI，`ysm::java` 提供签名、buffer 和 handle 机制，core 执行具体算法。Legacy 的 codec/container/conversion 有独立 C++ 结果边界，返回后仍进入 Java 当前容器管线。
...
|---|---|---|
| `Blake3`、`Zstd` / `ysm::algo` | 摘要、压缩或按期望大小/hash 验证的解压结果 | 选择 identity 输入、descriptor、业务预算与存储提交 |
